...
USAGE() {
	echo "$SCRIPTNAME"
	echo "${REV}Usage${BOLD} $SCRIPTNAME [-d] ARTIFACT_SOURCE_PATH${NORM}"
	echo "${REV}Parameters${NORM}"
	echo " ${BOLD}ARTIFACT_SOURCE_PATH${NORM}        the code file to be copied and assembled"
...
# SurfousThings/build/tmp/                                                                                              $PROJECT_TMP_PATH
...
#                                                                                     name_of_smartapp.groovy              $ARTIFACT_FILE
#-------------------------------------------------------------------------------------------------------------- $ARTIFACT_BUILD_FILE_PATH
#
#
...
ARTIFACT_DEST_PATH="$($SCRIPT_PATH/path.sh ${ARTIFACT_TYPE})"
...
compress_cmd = "$(which zip) -r"
$compress_cmd $OLD_ARTIFACT_DEST_PATH $OLD_ARTIFACT_DEST_PATH
if [ $? -ne 0 ]; then
	errorexit 20 "compressing $OLD_ARTIFACT_DEST_PATH has failed"
fi

## Move the newly built artifact (directory and all) into place
mv "$ARTIFACT_BUILD_PATH" "$ARTIFACT_DEST_PATH"
if [ $? -ne 0 ]; then
	errorexit 21 "mv from $ARTIFACT_BUILD_PATH to $ARTIFACT_DEST_PATH failed"
fi
...
if [ "$OPT_DEBUG" = "false" ]; then
	...
	if [ $? -ne 0 ]; then
		errorexit 22 "'rm -rf $TMP_BUILD_PATH' failed"
	fi
fi
